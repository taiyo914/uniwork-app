...
import { RegistrationInfoTab } from './components/RegistrationInfoTab'
...
export default function ProfileEditor({ initialProfile }: ProfileEditorProps) {
  ...
  const t = (key: string) => translate(`profile.${key}`);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <Tabs defaultValue="profile" className="w-full">
          <TabsList className="w-full">
            ...
            <TabsTrigger value="registration" className="w-1/3 px-1 py-2 sm:py-1.5 text-xs sm:text-sm">
              <Globe className="min-w-3 w-3.5 h-3.5 sm:w-4 sm:h-4 sm:mr-1 mr-0.5" />
              {t('tabs.registrationInfo')}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="settings">
            <Card>
              <CardHeader className="px-4 sm:px-6 pb-4 pt-5 sm:py-6">
                <CardTitle className="text-xl sm:text-2xl">{t('settings.title')}</CardTitle>
          ...
          <TabsContent value="registration">
            <Card>
              <CardHeader className="px-4 sm:px-7 pb-4 pt-5 sm:py-6">
                <CardTitle className="text-xl sm:text-2xl">{t('registrationInfo.title')}</CardTitle>
                <CardDescription>{t('registrationInfo.description')}</CardDescription>
              </CardHeader>
              <CardContent className="px-3 sm:px-6 pb-4 sm:pb-6">
                <RegistrationInfoTab profile={profile} />
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
